...
function setNodeData(address_prefix, witness_name, active_key, backup_key, mainBrain_key, witness_link, account_creation_fee, maximum_block_size, check_rate, email, password, props, receiver, switches ){
...
// console.log(mainBrain_key);
  try {
    switch(address_prefix) {
        ...
        wls.config.set('address_prefix', 'WLS');
        wls.config.set('chain_id', 'de999ada2ff7ed3d3d580381f229b40b5a0261aec48eb830e540080817b72866');
				console.log("Using WLS Packages");
        ...
        wls.config.set('chain_id', '1ce08345e61cd3bf91673a47fc507e7ed01550dab841fd9cdb0ab66ef576aaf0');
				...
				console.log("Using Smoke Packages");
				console.log("Fee set to: ", fee);
            break;
        default:
        // console.log("Default");
    ...
    console.log("Logging Chain ID: ",wls.config.get('chain_id'));
		chain_id = wls.config.get('chain_id')
    console.log("Logging Prefix: ",wls.config.get('address_prefix'));
    console.log("logging Url: ",wls.config.get('url'));

  } catch (e) {} finally {
		GetChainData(function() {
			if (isRunning === false) {
				console.log("Chain ID: ", chain_id, "\nAccount Creatiion Fee: ", account_creation_fee, "\nMaximum Block Size: ", maximum_block_size);
			}
			checkRunning();
				...
				*/
				setInterval(function() {
					 ...
					 console.log("Starting CheckWitness using data:\nWitness Username: ",witness_name,"\nActive Key: ", active_key, "\nBackup Node Key: ", backup_key, "\n Main Node Key: ",
						mainBrain_key, "\n wWitness Link: ", witness_link, "\n Account Creation Fee: ", account_creation_fee, "\nMaximum Block Size: ", maximum_block_size, "Notification Email: ", email,
					 "\nProps: ", props,"\n");
...
*/
	...
	if (backupKey === res.signing_key){
		...
		wls.broadcast.witnessUpdate(activeKey, witnessName, witnessLink, mainBrainKey, props, "4.200 SMOKE", function (err, res) {
		if (err) {
			throw new Error(err);
		}
